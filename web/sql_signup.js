...
<body>
  ...
  <!-- Script para validar y procesar el registro -->
  <script>
    // Función para generar MD5 (equivalente al MD5 de Java)
    function md5(string) {
    ...
    // Función para simular el guardado (aquí deberías integrar con tu backend)
    async function guardarUsuario(usuario) {
      // Simular una llamada a la API o backend
      return new Promise((resolve, reject) => {
        setTimeout(() => {
          // Simular verificación de usuario existente
          const usuariosExistentes = JSON.parse(localStorage.getItem('usuarios') || '[]');
          const usuarioExiste = usuariosExistentes.some(u => u.login === usuario.login);
    ...
    document.getElementById('formRegistro').addEventListener('submit', async function(e) {
      ...
      if (!nombre || !login || !password || !confirmPassword || !nivel) {
      ...
      try {
        ...
        const usuario = {
          nombre: nombre,
          login: login,
          password: md5(password), // Encriptar con MD5
          nivel: nivel,
          estado: 'activo',
        ...
        mostrarMensaje('Usuario registrado exitosamente. Redirigiendo al login...', 'success');
    ...
    // Validación del nombre de usuario (solo letras y números)
